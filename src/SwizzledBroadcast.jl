...
swizzle_style(::Broadcast.Style{Tuple}, dims, idims, op) = first(idims) == 1 ? Broadcast.Style{Tuple}() : Broadcast.DefaultArrayStyle(Val(first(idims)))
swizzle_style(style::A, dims, idims, op) where {N, A <: Broadcast.AbstractArrayStyle{N}} = A(Val(maximum(take(idims, N))))
swizzle_style(style::Broadcast.AbstractArrayStyle{Any}, dims, idims, op) = style
swizzle_style(::BroadcastStyle, dims, idims, op) = Broadcast.Unknown()
...
# in `dims` cannot be checked, since iterators may be infinitely large.

struct Swizzled{Style<:Union{Nothing,BroadcastStyle}, Axes, mask, imask, Arg, I, Op, N}
    arg::Arg
    dims::I
    op::Op
    axes::Axes
...
Swizzled(f::F, args::Args, axes=nothing) where {F, Args<:Tuple} =
...
### Objects with customized swizzling behavior should declare a BroadcastStyle for the Swizzle wrapper.

BroadcastStyle(::Type{<:Swizzle{<:Any, A}}) where {A} = BroadcastStyle(A)
