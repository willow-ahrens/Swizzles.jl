...
# `Swizzled` wrap the arguments to `swizzle(arg, dims=(), op=....)`. A statement like
#    y = Swizzler(+, 2).(x .* (x .+ 1))
# will result in code that is essentially
#    y = copy(Swizzled(Broadcasted(*, x, Broadcasted(+, x, 1)), (2,), +))
...
#    copyto!(dest, sz::Swizzled) = copyto!(dest, convert(Swizzled{Nothing}, sz))
# This allows methods
#    copyto!(dest::DestType,  sz::Swizzled{Nothing})
# that specialize on `DestType` to be easily disambiguated from
# methods that instead specialize on `BroadcastStyle`,
#    copyto!(dest::AbstractArray, sz::Swizzled{MyStyle})

struct Swizzled{Style<:Union{Nothing,BroadcastStyle}, Axes, dims, idims, Arg, Op, N}
    arg::Arg
    Op::op
    axes::Axes          # the axes of the resulting object (may be bigger than implied by `args` if this is nested inside a larger `Swizzled`)
    #TODO enforce inv swizzle params
end

@inline swizzle_dims(sw::Swizzled{Style, Axes, N, Arg, dims}) where {Style,Axes,N,Arg,dims} = dims
...
Base.convert(::Type{Swizzled{NewStyle}}, sz::Swizzled{Style,Axes,N,Arg,dims,idims,Op}) where {NewStyle,Style,Axes,N,Arg,dims,idims,Op} =
    Swizzled{Style,Axes,N,Arg,dims,idims,Op}(sz.arg, sz.op, sz.axes)

function Base.show(io::IO, sw::Swizzled{Style, Axes, N, Arg, dims}) where {Style,Axes,N,Arg,dims}
    print(io, Swizzled)
    # Only show the style parameter if we have a set of axes — representing an instantiated
    # "outermost" Swizzled. The styles of nested Swizzleds represent an intermediate
    # computation that is not relevant for dispatch, confusing, and just extra line noise.
    swizzled.axes isa Tuple && print(io, '{', Style, '}')
    if Op isa typeof(unspecifiedop)
      print(io, '(', bc.arg, ", ", swizzle_dims(sz), ')')
    else
      print(io, '(', bc.arg, ", ", swizzle_dims(sz), ", ", op, ')')
    end
    nothing
end
...
@inline _axes(sz::Swizzled, ::Nothing) = apply_swizzle_dims(broadcast_axes(sz.arg), Base.OneTo(1), swizzle_dims(sz))

BroadcastStyle(::Type{<:Swizzled{Style}}) where {Style} = Style()
BroadcastStyle(::Type{<:Swizzled{S}}) where {S<:Union{Nothing,Unknown}} =
    throw(ArgumentError("Swizzle{Unknown} wrappers do not have a style assigned"))

argtype(::Type{Swizzled{<:Any,<:Any,<:Any,Arg}}) where {Arg} = Arg
...
## Instantiation fills in the "missing" fields in Broadcasted. #TODO REMOVE
#instantiate(x) = x #TODO REMOVE

"""
    Broadcast.instantiate(sz::Swizzled)
Construct and check the axes for the lazy Swizzled object `sz`.
...
@inline function instantiate(sz::Swizzled{Style}) where {Style}
    if sz.axes isa Nothing # Not done via dispatch to make it easier to extend instantiate(::Swizzled{Style})
        axes = apply_swizzle_dims(broadcast_axes(sz.arg), swizzle_dims(sz))
    else
        axes = sz.axes
        check_swizzle_axes(axes, swizzle_dims(sz), sz.arg)
    end
    Swizzled{Style}(arg, swizzle_dims(sz), sz.op, axes)
end
#instantiate(sz::Swizzled{<:Union{AbstractArrayStyle{0}, Style{Tuple}}}) = 

## We skip Flattening, it doesn't make sense to flatten Swizzles, don't flatten Swizzles. In fact, stop flattening in general. Bad. Stop. Stop thinking about it.

## Broadcasting utilities ##

swizzle_style(arg, dims, op) = combine_styles(arg)
